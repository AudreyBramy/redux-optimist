...
function optimist(fn) {
  function beginReducer(state, action) {
    ...
    var innerStateCopy = Object.assign({}, innerState);
    optimist = optimist.concat([{ beforeState: innerStateCopy, action: action }]);
    innerStateCopy = fn(innerStateCopy, action);
    validateState(innerStateCopy, action);
    return _extends({ optimist: optimist }, innerStateCopy);
  }
